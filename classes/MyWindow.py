            ...
            for line in self.board:
                for new_square in line:
                    if new_square.returnPoint(x, y):  # verificar onde foi clicado
                        i_final, j_final = new_square.returnCoordinates()  # coordenadas novas/finais
                        (i_initial, j_initial) = self.gamestate.clicked  # receber as coordenadas do gamestate para comparação
                        ...
                        if old_square.analyseMove(new_square, self.board):  # verificar se é possível mover
                            ...
                            new_square.changeImageCoord(i_final, j_final)  # mudar as coordenadas do sprite da peça
